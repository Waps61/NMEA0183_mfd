...
void mfd_config_panel_create(lv_obj_t *parent)
{
  lv_log("creating panel %s\n",lv_obj_get_name(parent));
    ...
    lv_obj_t *baudvalue = lv_obj_create(parent);
    ...
    lv_subject_set_int(&mfd_subject_wifi, ship_config.wifi_on);
    ...
    if(ship_config.wifi_on)
      ...
      lv_subject_set_float(&mfd_subject_log, ship_config.ship_log);
      ...
      lv_label_bind_text(logvalue, &mfd_subject_log,"%.1f");
